...
func ConfigureClient(p *print.Printer) (*skcf.APIClient, error) {
	var err error
	var apiClient *skcf.APIClient
	...
	customEndpoint := viper.GetString(config.SKCFCustomEndpointKey)
	if customEndpoint != "" {
	...
	apiClient, err = skcf.NewAPIClient(cfgOptions...)
	if err != nil {
