...
func NewCmd(p *print.Printer) *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Updates an SKCF cluster",
		Long: fmt.Sprintf("%s\n%s\n%s",
			"Updates a STACKIT Kubernetes Engine (SKCF) cluster.",
		...
		Example: examples.Build(
			examples.NewExample(
				`Update an SKCF cluster using an API payload sourced from the file "./payload.json"`,
				"$ stackit skcf cluster update my-cluster --payload @./payload.json"),
			examples.NewExample(
				`Update an SKCF cluster using an API payload provided as a JSON string`,
				`$ stackit skcf cluster update my-cluster --payload "{...}"`),
			examples.NewExample(
				...
				`$ stackit skcf cluster generate-payload --cluster-name my-cluster > ./payload.json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("update SKCF cluster: %w", err)
			}
			name := *resp.Name
			...
			if !model.Async {
				...
				if err != nil {
					return fmt.Errorf("wait for SKCF cluster update: %w", err)
				}
				s.Stop()
			}
...
func outputResult(p *print.Printer, model *inputModel, resp *skcf.Cluster) error {
