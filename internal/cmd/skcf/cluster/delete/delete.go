...
func NewCmd(p *print.Printer) *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Deletes a SKCF cluster",
		Long:  "Deletes a STACKIT Korifi Cloud Foundry (SKCF) cluster.",
		Args:  args.SingleArg(clusterNameArg, nil),
		Example: examples.Build(
			examples.NewExample(
				`Delete an SKCF cluster with name "my-cluster"`,
				"$ stackit skcf cluster delete my-cluster"),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = req.Execute()
			if err != nil {
				return fmt.Errorf("delete SKCF cluster: %w", err)
			}
			...
			if !model.Async {
				...
				if err != nil {
					return fmt.Errorf("wait for SKCF cluster deletion: %w", err)
				}
				s.Stop()
			}
...
func buildRequest(ctx context.Context, model *inputModel, apiClient *skcf.APIClient) skcf.ApiDeleteClusterRequest {
