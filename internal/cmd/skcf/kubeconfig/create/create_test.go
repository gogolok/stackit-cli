...
func fixtureRequest(mods ...func(request *skcf.ApiCreateKubeconfigRequest)) skcf.ApiCreateKubeconfigRequest {
	request := testClient.CreateKubeconfig(testCtx, testProjectId, testClusterName)
	request = request.CreateKubeconfigPayload(skcf.CreateKubeconfigPayload{})
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func TestBuildRequestCreate(t *testing.T) {
	tests := []struct {
		description     string
		model           *inputModel
		expectedRequest skcf.ApiCreateKubeconfigRequest
	}{
		{
			description:     "base",
		...
		},
		{
			...
			expectedRequest: fixtureRequest().CreateKubeconfigPayload(skcf.CreateKubeconfigPayload{
