...
func NewCmd(p *print.Printer) *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Creates a kubeconfig for an SKCF cluster",
		Long: fmt.Sprintf("%s\n\n%s\n%s\n%s\n%s",
			"Creates a kubeconfig for a STACKIT Korifi Cloud Foundry (SKCF) cluster.",
		...
		Example: examples.Build(
			examples.NewExample(
				`Create a kubeconfig for the SKCF cluster with name "my-cluster"`,
				"$ stackit skcf kubeconfig create my-cluster"),
			examples.NewExample(
				`Get a login kubeconfig for the SKCF cluster with name "my-cluster". `+
					"This kubeconfig does not contain any credentials and instead obtains valid credentials via the `stackit skcf kubeconfig login` command.",
				"$ stackit skcf kubeconfig create my-cluster --login"),
			examples.NewExample(
				`Create a kubeconfig for the SKCF cluster with name "my-cluster" and set the expiration time to 30 days`,
				"$ stackit skcf kubeconfig create my-cluster --expiration 30d"),
			examples.NewExample(
				`Create a kubeconfig for the SKCF cluster with name "my-cluster" and set the expiration time to 2 months`,
				"$ stackit skcf kubeconfig create my-cluster --expiration 2M"),
			examples.NewExample(
				`Create a kubeconfig for the SKCF cluster with name "my-cluster" in a custom filepath`,
				"$ stackit skcf kubeconfig create my-cluster --filepath /path/to/config"),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !model.AssumeYes {
				prompt := fmt.Sprintf("Are you sure you want to create a kubeconfig for SKCF cluster %q? This will OVERWRITE your current kubeconfig file, if it exists.", model.ClusterName)
			...
			var (
				kubeconfig     string
				respKubeconfig *skcf.Kubeconfig
				respLogin      *skcf.LoginKubeconfig
			)
			...
			if !model.Login {
				...
				if err != nil {
					return fmt.Errorf("create kubeconfig for SKCF cluster: %w", err)
				}
				...
				if err != nil {
					return fmt.Errorf("create login kubeconfig for SKCF cluster: %w", err)
				}
			...
			if model.Filepath == nil {
				kubeconfigPath, err = skcfUtils.GetDefaultKubeconfigPath()
				if err != nil {
			...
			if model.OutputFormat != print.JSONOutputFormat {
				err = skcfUtils.WriteConfigFile(kubeconfigPath, kubeconfig)
				if err != nil {
...
func parseInput(p *print.Printer, cmd *cobra.Command, inputArgs []string) (*inputModel, error) {
	...
	if expTime != nil {
		var err error
		expTime, err = skcfUtils.ConvertToSeconds(*expTime)
		if err != nil {
...
func buildRequestCreate(ctx context.Context, model *inputModel, apiClient *skcf.APIClient) (skcf.ApiCreateKubeconfigRequest, error) {
	req := apiClient.CreateKubeconfig(ctx, model.ProjectId, model.ClusterName)

	payload := skcf.CreateKubeconfigPayload{}
...
func buildRequestLogin(ctx context.Context, model *inputModel, apiClient *skcf.APIClient) (skcf.ApiGetLoginKubeconfigRequest, error) {
	return apiClient.GetLoginKubeconfig(ctx, model.ProjectId, model.ClusterName), nil
}

func outputResult(p *print.Printer, model *inputModel, kubeconfigPath string, respKubeconfig *skcf.Kubeconfig, respLogin *skcf.LoginKubeconfig) error {
	...
	case print.JSONOutputFormat:
		...
		if err != nil {
			return fmt.Errorf("marshal SKCF Kubeconfig: %w", err)
		}
