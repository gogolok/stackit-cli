...
func NewCmd(p *print.Printer) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf("%s\n%s\n%s",
			"Login plugin for kubernetes clients, that creates short-lived credentials to authenticate against a STACKIT Korifi Cloud Foundry (SKCF) cluster.",
		...
		Example: examples.Build(
			...
			examples.NewExample(
				"Use the previously saved kubeconfig to authenticate to the SKCF cluster, in this case with kubectl.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if env == "" {
				return fmt.Errorf("%s\n%s\n%s", "KUBERNETES_EXEC_INFO env var is unset or empty.",
					...
					"See `stackit skcf kubeconfig login --help` for detailed usage instructions.")
			}
...
func parseClusterConfig() (*clusterConfig, error) {
	...
	config.cacheKey = fmt.Sprintf("skcf-login-%x", sha256.Sum256([]byte(execCredential.Spec.Cluster.Server)))
...
func GetAndOutputKubeconfig(ctx context.Context, p *print.Printer, apiClient *skcf.APIClient, clusterConfig *clusterConfig, fallbackToCache bool, cachedKubeconfig *rest.Config) error {
...
func buildRequest(ctx context.Context, apiClient *skcf.APIClient, clusterConfig *clusterConfig) skcf.ApiCreateKubeconfigRequest {
	...
	return req.CreateKubeconfigPayload(skcf.CreateKubeconfigPayload{ExpirationSeconds: &expirationSeconds})
}
